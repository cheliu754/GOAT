...
type CustomField = { id: string; label: string; type: CustomFieldType; value: string | number | boolean; };
...
export default function UpdateModal() {
  ...
  // Sample form state (same as before; wire to data later)
  ...
  const [customFields, setCustomFields] = useState<CustomField[]>([
    { id: crypto.randomUUID(), label: "SAT Requirement", type: "number", value: 1500 },
    { id: crypto.randomUUID(), label: "Enable Notification", type: "flag", value: true },
  ]);
  ...
  const addCustomField = () => {
    ...
    const defaultValue = newType === "text" ? "" : newType === "number" ? 0 : false;
    setCustomFields((prev) => [...prev, { id, label: newLabel.trim(), type: newType, value: defaultValue }]);
  ...
  const updateFieldValue = (id: string, v: string | number | boolean) =>
    setCustomFields((prev) => prev.map((f) => (f.id === id ? { ...f, value: v } : f)));
    ...
    <div className="modalBackdrop" onClick={close} aria-hidden>
      ...
      >
        <header className="modal__head">
          <h1 id="upd-title" className="modal__title">Update Record</h1>
          <button className="iconbtn" aria-label="Close" onClick={close}>✕</button>
        </header>
        ...
        <div className="modal__content">
          <div className="panel">
            ...
            <div className="grid2">
              <label className="field">
                ...
                <input className="field__input" type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} />
              </label>
            ...
            <label className="field">
              ...
              <input className="field__input" type="url" value={website} onChange={(e) => setWebsite(e.target.value)} />
            </label>

            <label className="field">
              <span className="field__label">Notes</span>
              <textarea className="field__input field__textarea" rows={4} value={notes} onChange={(e) => setNotes(e.target.value)} />
            </label>
          </div>
          ...
          <div className="panel">
            ...
            <div className="adder">
              ...
              <button className="btn" onClick={addCustomField} disabled={isAddDisabled} type="button">
                Add More
              </button>
            </div>
              ...
              {customFields.map((f) => (
                <li key={f.id} className="extra">
                  <div className="extra__row">
                    ...
                    <button className="iconbtn" onClick={() => removeCustomField(f.id)} aria-label={`Remove ${f.label}`}>✕</button>
                  </div>
                  ...
                  {f.type === "number" && (
                    <input
                      ...
                      onChange={(e) => updateFieldValue(f.id, Number(e.target.value))}
                    />
                  )}
                  {f.type === "flag" && (
                    <label className="toggle">
                      <input
                        ...
                        onChange={(e) => updateFieldValue(f.id, e.target.checked)}
                      />
                      <span className="toggle__ui" />
                      <span className="toggle__text">{Boolean(f.value) ? "Enabled" : "Disabled"}</span>
                    </label>
                  )}
                </li>
              ))}
        ...
        <footer className="modal__footer">
          <button className="btn btn--light" type="button" onClick={close}>Cancel</button>
